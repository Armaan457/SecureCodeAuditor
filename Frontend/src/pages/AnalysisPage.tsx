...
const handleProcess = async () => {
  ...
  const handleDownload = () => {
    ...
    const filesScanned = Object.keys(analysisResults).length;
    let total = 0;
    let critical = 0;
    let high = 0;
    let medium = 0;
    ...
    Object.values(analysisResults).forEach(findings => {
      findings.forEach(f => {
        total++;
    ...
    doc.setFontSize(28);
    doc.text("SecureCodeAuditor", 105, 18, {
      align: "center",
    });

    doc.setFontSize(13);
    doc.setFont("helvetica", "normal");
    doc.text("AI Powered Security Analysis", 105, 28, {
      align: "center",
    });

    doc.setDrawColor(0, 120, 0);
    doc.setLineWidth(0.5);
    doc.roundedRect(15, 50, 180, 40, 4, 4);

    doc.setTextColor(0);

    doc.setFont("helvetica", "bold");
    doc.setFontSize(13);
    doc.text("Repository", 20, 62);

    doc.setFont("helvetica", "normal");
    doc.setFontSize(11);

    const repoLines = doc.splitTextToSize(githubUrl, 165);
    doc.text(repoLines, 20, 70);

    doc.setFont("helvetica", "bold");
    doc.text("Generated", 20, 84);

    doc.setFont("helvetica", "normal");
    doc.text(new Date().toLocaleString(), 55, 84);

    // Summary box
    doc.setDrawColor(0, 120, 0);
    doc.roundedRect(15, 105, 180, 90, 4, 4);

    doc.setFont("helvetica", "bold");
    doc.setFontSize(16);
    doc.text("Executive Summary", 20, 118);
    ...
    const addRow = (label: string, value: string | number) => {
      doc.text(label, 25, summaryY);
      doc.text(String(value), 170, summaryY, {
        align: "right",
      });
      summaryY += 10;
    };

    addRow("Files Scanned", filesScanned);
    addRow("Total Findings", total);

    summaryY += 5;

    addRow("Critical", critical);
    addRow("High", high);
    addRow("Medium", medium);
    addRow("Low", low);
    ...
    doc.text(
      "Generated by SecureCodeAuditor",
      105,
      285,
      {
        align: "center",
    ...
    Object.entries(analysisResults).forEach(([filename, findings]) => {
      // Every file starts on a fresh page after the cover page
      doc.addPage();
      ...
      const fileLines = doc.splitTextToSize(filename, 180);
      doc.text(fileLines, 14, 20);

      if (!findings.length) {
        doc.setFont("helvetica", "normal");
        doc.setFontSize(12);
        doc.text("No vulnerabilities found.", 14, 35);
        return;
      }

      autoTable(doc, {
        startY: 20 + fileLines.length * 7 + 6,
      ...
      findings.forEach((finding, index) => {
        ...
        doc.text(`${filename} - Finding ${index + 1}`, 14, 18);
        ...
        addSection("Vulnerability", finding.vulnerability_type);
        addSection("Severity", finding.severity);
        addSection("Confidence", finding.confidence);
        addSection("Reason", finding.reason);
        addSection("Recommendation", finding.recommendation);
        addSection("Code Snippet", finding.code_snippet);
      });
    });

    doc.save(`security-report-${repoSlug}.pdf`);
      };
        ...
        <section className="max-w-2xl mx-auto mb-8 reveal reveal-delay-1">
          <div className="p-6 border border-green-500/20 rounded-xl bg-green-500/5 backdrop-blur-sm hover:shadow-[0_0_15px_rgba(34,197,94,0.2)] transition-all duration-300">
            ...
            <p className="text-xs text-green-500/50 mt-3">
              Public repositories work best. Private repositories require access credentials.
            </p>
            </div>
        </section>
